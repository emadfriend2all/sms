...
using Showmatics.Infrastructure.Common.Extensions;

namespace Showmatics.Infrastructure.FileStorage;
...
public class LocalFileStorageService : IFileStorageService
{
    ...
    where T : class
    {
        ...
        string base64Data = (supportedFileType == FileType.Image) ?
            Regex.Match(request.Data, "data:image/(?<type>.+?),(?<data>.+)").Groups["data"].Value :
            Regex.Match(request.Data, "data:application/(?<type>.+?),(?<data>.+)").Groups["data"].Value;
