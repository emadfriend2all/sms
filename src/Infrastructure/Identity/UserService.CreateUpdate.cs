...
using Showmatics.Application.Common.Mailing;
using Showmatics.Application.Identity.Users;
using Showmatics.Domain.Common;
using Showmatics.Domain.Identity;
...
internal partial class UserService
{
    ...
    private async Task<ApplicationUser> CreateOrUpdateFromPrincipalAsync(ClaimsPrincipal principal)
    {
        ...
        {
            throw new InternalServerException(string.Format(_t["identity.userNameorEmailAlreadyTaken"]));
        }

        var user = await _userManager.FindByNameAsync(username);
        if (user is not null && !string.IsNullOrWhiteSpace(user.ObjectId))
        {
            throw new InternalServerException(string.Format(_t["identity.usernameAlreadyTaken"], username));
        }
        ...
        if (user is null)
        {
            ...
            if (user is not null && !string.IsNullOrWhiteSpace(user.ObjectId))
            {
                throw new InternalServerException(string.Format(_t["identity.emailAlreadyTaken"], email));
            }
        }
    ...
    public async Task<string> CreateAsync(CreateUserRequest request, string origin)
    {
        ...
        var messages = new List<string> { string.Format(_t["identity.userIsRegistered"], user.UserName) };
        ...
        if (_securitySettings.RequireConfirmedAccount && !string.IsNullOrEmpty(user.Email))
        {
            ...
            var mailRequest = new MailRequest(
                ...
                _t["identity.confirmRegistration"],
                _templateService.GenerateEmailTemplate("email-confirmation", eMailModel));
            _jobService.Enqueue(() => _mailService.SendAsync(mailRequest, CancellationToken.None));
            messages.Add(_t[$"identity.checkemailtoVerifyAccount"]);
        }
    ...
    public async Task UpdateAsync(UpdateUserRequest request, string userId)
    {
        ...
        _ = user ?? throw new NotFoundException(_t["identity.usernotfound"]);
        ...
        if (!result.Succeeded)
        {
            throw new InternalServerException(_t["identity.updateProfileFailed"], result.GetErrors(_t));
        }
    }
}
