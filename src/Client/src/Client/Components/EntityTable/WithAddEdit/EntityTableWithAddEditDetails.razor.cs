using Showmatics.Blazor.Client.Components.Dialogs;
...
public partial class EntityTableWithAddEditDetails<TEntity, TId, TAddRequest, TEditRequest, TDetailsRequest>
    where TAddRequest : new()
    where TEditRequest : new()
    where TDetailsRequest : new()
{
    [Parameter]
    [EditorRequired]
    public EntityTableWithAddEditDetailsContext<TEntity, TId, TAddRequest, TEditRequest, TDetailsRequest> Context { get; set; } = default!;
    ...
    [Parameter]
    public RenderFragment<TEditRequest>? EditFormContent { get; set; }

    [Parameter]
    public RenderFragment<TAddRequest>? AddFormContent { get; set; }

    [Parameter]
    public RenderFragment<TAddRequest>? DetailsFormContent { get; set; }
    ...
    private async Task InvokeEditModal(TEntity? entity = default)
    {
        bool isCreate = entity is null;

        var parameters = new DialogParameters()
        {
            { nameof(AddEditModal<TEditRequest>.EditFormContent), EditFormContent },
            { nameof(AddEditModal<TEditRequest>.OnInitializedFunc), Context.EditFormInitializedFunc },
            { nameof(AddEditModal<TEditRequest>.EntityName), Context.EntityName }
        };

        TEditRequest editRequestModel;

        _ = Context.IdFunc ?? throw new InvalidOperationException("IdFunc can't be null!");
        var id = Context.IdFunc(entity!);
        parameters.Add(nameof(AddEditModal<TEditRequest>.Id), id);

        _ = Context.UpdateFunc ?? throw new InvalidOperationException("UpdateFunc can't be null!");
        Func<TEditRequest, Task> saveFunc = entity => Context.UpdateFunc(id, entity);
        parameters.Add(nameof(AddEditModal<TEditRequest>.SaveFunc), saveFunc);

        editRequestModel =
            Context.GetForEditFunc is not null
                && await ApiHelper.ExecuteCallGuardedAsync(
                        () => Context.GetForEditFunc!(id!),
                        Snackbar)
                    is { } detailsResult
            ? detailsResult
            : entity!.Adapt<TEditRequest>();

        parameters.Add(nameof(AddEditModal<TEditRequest>.RequestModel), editRequestModel);

        var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Medium, FullWidth = true, DisableBackdropClick = true };

        var dialog = DialogService.Show<AddEditModal<TEditRequest>>(string.Empty, parameters, options);

        Context.SetEditModalRef(dialog);
    ...
    private async Task InvokeAddModal(TEntity? entity = default)
    {
        bool isCreate = entity is null;

        var parameters = new DialogParameters()
        {
            { nameof(AddEditModal<TAddRequest>.EditFormContent), AddFormContent },
            { nameof(AddEditModal<TAddRequest>.OnInitializedFunc), Context.AddFormInitializedFunc },
            { nameof(AddEditModal<TAddRequest>.EntityName), Context.EntityName }
        };

        _ = Context.CreateFunc ?? throw new InvalidOperationException("CreateFunc can't be null!");
        parameters.Add(nameof(AddEditModal<TAddRequest>.SaveFunc), Context.CreateFunc);
        TAddRequest requestModel =
        ...
        parameters.Add(nameof(AddEditModal<TAddRequest>.RequestModel), requestModel!);

        var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Medium, FullWidth = true, DisableBackdropClick = true };

        var dialog = DialogService.Show<AddEditModal<TAddRequest>>(string.Empty, parameters, options);

        Context.SetAddModalRef(dialog);
