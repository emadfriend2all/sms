...
/// <typeparam name="TAddRequest">The type of the Request which is used on the AddModal and which is sent with the CreateFunc.</typeparam>
/// <typeparam name="TEditRequest">The type of the Request which is used on the EditModal and which is sent with the UpdateFunc.</typeparam>
/// <typeparam name="TDetailsRequest">The type of the Request which is used on the DetailsModal and which is sent with the DetailsFunc.</typeparam>
public abstract class EntityTableWithAddEditDetailsContext<TEntity, TId, TAddRequest, TEditRequest, TDetailsRequest>
{
    /// <summary>
    ...
    /// </summary>
    public Func<TId, Task<TDetailsRequest>>? GetDetailsFunc { get; }
    ...
    /// When not supplied, the TEntity out of the _entityList is supplied using the IdFunc and converted using mapster.
    /// </summary>
    public Func<TId, Task<TEditRequest>>? GetForEditFunc { get; }
    ...
    /// No need to check for error messages or api exceptions. These are automatically handled by the component.
    /// </summary>
    public Func<TId, TEditRequest, Task>? UpdateFunc { get; }
    ...
    /// Use this if you want to run initialization during OnInitialized of the AddEdit form.
    /// </summary>
    public Func<Task>? AddFormInitializedFunc { get; }
    ...
    public EntityTableWithAddEditDetailsContext(
        List<EntityField<TEntity>> fields,
        Func<TEntity, TId>? idFunc,
        Func<Task<TAddRequest>>? getDefaultsFunc,
        Func<TAddRequest, Task>? createFunc,
        Func<TId, Task<TDetailsRequest>>? getDetailsFunc,
        Func<TId, Task<TEditRequest>>? getForEditFunc,
        Func<TId, TEditRequest, Task>? updateFunc,
    ...
    // AddEdit modal
    private IDialogReference? _addModalRef;

    // AddEdit modal
    private IDialogReference? _editModalRef;

    internal void SetAddModalRef(IDialogReference dialog) =>
        _addModalRef = dialog;

    internal void SetEditModalRef(IDialogReference dialog) =>
        _editModalRef = dialog;

    public IAddEditModal<TAddRequest> AddModal =>
        _addModalRef?.Dialog as IAddEditModal<TAddRequest>
        ?? throw new InvalidOperationException("AddModal is only available when the modal is shown.");
    public IAddEditModal<TEditRequest> EditModal =>
        _editModalRef?.Dialog as IAddEditModal<TEditRequest>
        ?? throw new InvalidOperationException("EditModal is only available when the modal is shown.");

    // Shortcuts
    public EntityClientTableContext<TEntity, TId, TAddRequest>? ClientContext => this as EntityClientTableContext<TEntity, TId, TAddRequest>;
    public EntityServerTableWithAddEditDetailsContext<TEntity, TId, TAddRequest, TEditRequest, TDetailsRequest>? ServerContext => this as EntityServerTableWithAddEditDetailsContext<TEntity, TId, TAddRequest, TEditRequest, TDetailsRequest>;
