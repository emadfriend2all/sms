...
namespace Showmatics.Blazor.Client.Infrastructure.Auth.AzureAd;
...
internal class AzureAdClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
    ...
    public override async ValueTask<ClaimsPrincipal> CreateUserAsync(RemoteUserAccount account, RemoteAuthenticationUserOptions options)
    {
        ...
        if (principal.Identity?.IsAuthenticated is true)
        {
            ...
            if (!userIdentity.HasClaim(c => c.Type == ClaimTypes.GivenName))
            {
                userIdentity.AddClaim(new Claim(ClaimTypes.GivenName, userDetails.UserName ?? string.Empty));
            }
